...
class PendudukController extends Controller
{
    ...
    public function uploadfoto(Request $request)
    {
        ...
        $foto_penduduk = $request->file('foto_penduduk');
        $name = date('Ymd').'-'.time().'.'.$foto_penduduk->getClientOriginalExtension();
        $destinationPath = public_path('penduduk');
        $foto_penduduk->move($destinationPath, $name);

        $insert = DB::table('penduduks')->where('id',$id_penduduk)->update(
            [
                'foto_penduduk' => $name,
            ]
        );
